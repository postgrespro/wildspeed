...
/*
 * MARK_SIGN is a sign of the end of a string and this character should
 * be a regular character. The best candidate of this is a zero byte
 * which is accepted for any locale used by Postgres. But it's impossible
 * to show it, so we will replace it with another one (MARK_SIGN_SHOW) which
 * can be noticed well. But we can't use it as a mark because it's allowed
...
#define WC_BEGIN		0x01	/* should be at the begining of the string */
#define WC_MIDDLE		0x02	/* should be in the middle of the string */
#define WC_END			0x04	/* should be at the end of the string */
...
/*
 * setFlagOfText
 *		Mark text by setting the first byte to MARK_SIGN to indicate that text
 *		has flags. It's safe for a non-empty string because the first character
 *		can not be MARK_SIGN (see gin_extract_permuted())
 */
static text *
 ...
 * 		Returns all permuterms for the string as an array of keys to be indexed.
 */
...
gin_extract_permuted(PG_FUNCTION_ARGS)
{
	...
	*nentries = srclen;
	...
	else
	{
		...
		/*
		 * Permutation: hello will be permuted to hello$, ello$h, llo$he,
		 * lo$hel, o$hell. So, number of entries is equal to number of
		...
		for (i = 0; i < srclen; i++)
		{
			...
			/*
			 * Copy the first part. For llo$he it will be 'llo'
			 */
			...
			/*
			 * Copy the rest of string (in example above 'he')
			 */
...
wildcmp_internal(text *a, text *b, bool partialMatch)
{
	...
	/*
	 * Sets correct pointers and lengths in case of flags presence
	 */
	if (lena > 2 && *ptra == MARK_SIGN)
	{
		...
		if (lenb > 2 && *ptrb == MARK_SIGN)
		{
			 ...
			 * place works only during check of equality of keys to search
			 */
			if (flag != *(ptrb + 1))
				return 1;
			ptrb += 2;
			...
			/*
			 * b can not be a product of gin_extract_wildcard for partial
			 * match mode
			 */
	...
	{
		/*
		 * b can not be a product of gin_extract_wildcard for partial match
		 * mode
		 */
		Assert(partialMatch == false);
		...
		if (partialMatch)
		{
			if (cmp == 0)
			{
				...
				{				/* there is some flags to check */
					...
					{
						/*
						 * Prefix are matched but this prefix s not placed as
						 * needed. So, we should give a smoke signal to GIN
						 * that we don't want this match, but wish to continue
						 * scan
						 */
						cmp = -1;
					}
...
gin_extract_wildcard(PG_FUNCTION_ARGS)
{
	...
	*nentries = 0;
	...
	/*
	 * Parse expression to the list of constant parts and wildcards
	 */
	while (qptr - VARDATA(q) < lenq)
	{
		...
		if (clen == 1 && (*qptr == '_' || *qptr == '%'))
		{
			...
			/*
			 * ignore wildcard, because we don't make difference beetween %, _
	...
	if (items[0].iswildcard == false)
	{
		...
		else if (items[splitqlen - 1].iswildcard == false)
		{
			/* X * [X1 * [] ] ] Y => Y$X* [ + X1* [] ] */
			*nentries = 1;
			entry = appendStrToText(NULL,
									items[splitqlen - 1].ptr,
									items[splitqlen - 1].len,
									lenq + 1);
			entry = appendMarkToText(entry, -1);
			...
			for (i = 1; i < splitqlen - 1; i++)
			{
				if (items[i].iswildcard)
					continue;
				/* length = lenq + 1 (for MARK_SIGN) + 2 (for flag) */
				entry = setFlagOfText(WC_MIDDLE, lenq + 1 + 2);
		...
		else
		{
			/* X * [ X1 * [] ]  => X*$ [ + X1* [] ] */
			/* length = lenq + 1 (for MARK_SIGN) + 2 (for flag) */
			entry = setFlagOfText(WC_BEGIN, lenq + 1 + 2);
			entry = appendStrToText(entry, items[0].ptr, items[0].len, -1);
			*nentries = 1;
			partialmatch[0] = true;
			entries[0] = PointerGetDatum(entry);
		...
		else
		{
			...
			for (i = 1; i < splitqlen - 1; i++)
			{
				...
				if (splitqlen > 3)
				{
					/* length = lenq + 1 (for MARK_SIGN) + 2 (for flag) */
					if (i == 1)
						entry = setFlagOfText(WC_MIDDLE | WC_BEGIN, lenq + 1 + 2);
					else if (i == splitqlen - 2)
						entry = setFlagOfText(WC_MIDDLE | WC_END, lenq + 1 + 2);
					else
						entry = setFlagOfText(WC_MIDDLE, lenq + 1 + 2);
				}
				else
	...
	{
		/*
		 * Non empty extra signals to consistentFn about rechecking of result
		 */
...
/*
 * gin_triconsistent_wildcard
 *		ternanry variant of consistent function
 */
PG_FUNCTION_INFO_V1(gin_triconsistent_wildcard);
Datum
gin_triconsistent_wildcard(PG_FUNCTION_ARGS)
{
	...
	for (i = 0; res != GIN_FALSE && i < nentries; i++)
		if (DatumGetGinTernaryValue(check[i]) == GIN_FALSE)
			res = GIN_FALSE;

	/* need extra recheck */
	if (res == GIN_TRUE && extra != NULL)
		res = GIN_MAYBE;
...
permute(PG_FUNCTION_ARGS)
{
	...
	/*
	 * We need to replace MARK_SIGN to MARK_SIGN_SHOW. See comments above near
	 * definition of MARK_SIGN and MARK_SIGN_SHOW.
	 */
	...
	else
	{
		...
		/*
		 * We scan array from the end because it allows simple calculation of
		 * MARK_SIGN position: on every iteration it's moved one character to
		 * the end.
		 */
